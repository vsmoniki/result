...
ブラウザで表示されたURLを開き、FITまたはCSVファイルを選択して必要項目を入力してください。CSVはヘッダー付きで、1秒ごとのパワーと心拍だけのデータでも各行を1秒サンプルとして扱い、平均パワー、最大パワー、最大心拍、ベスト平均パワー、グラフを計算します。時刻または経過時間、ケイデンス、距離、速度などの列がある場合は自動判別して利用します。カンマ区切り、セミコロン区切り、タブ区切りに対応しています。
